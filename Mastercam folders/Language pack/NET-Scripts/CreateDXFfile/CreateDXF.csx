// -----------------------------------------------------------------------------------------
// <copyright file="CreateDXF.csx" company="CNC Software, LLC">
...
///<summary> The messagebox title </summary>
private const string Title = "Create DXF";
...
public class App
{
	...
	public void Run()
	{
		...
		{
			DialogManager.Error(success.Error, Title);
			return;
		}

		GraphicsManager.FitScreen();

		var path = SelectFiles("Select file location");
		if (path.IsFailure)
		{
			DialogManager.Error(path.Error, Title);
			return;
		}

		var saved = FileManager.SaveAs(path.Value, false);
		if (saved)
		{
			// clear screen
			FileManager.New();

			DialogManager.OK("File Saved", Title);
		}
	}

#region Private Methods

	/// <summary> Select one a more json files. </summary>
	///
	/// <param name="title"> Dialog description </param>
	///
	/// <returns> A list of selected json files </returns>
	private Result<string> SelectFiles(string title)
	{
		using (var browse = new SaveFileDialog())
		{
			browse.Title = title;
			browse.Filter = @"DXF files(*.dxf)| *.dxf";

			var result = browse.ShowDialog();
			if (result == DialogResult.OK)
			{
				return Result.Ok(browse.FileName);
			}
		}
